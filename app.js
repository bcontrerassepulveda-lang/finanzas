...
const seedCategories = ["Sueldo", "Trabajo extra", "Honorarios", "Venta", "Bono", "Intereses", "Comida", "Transporte", "Vivienda", "Salud", "Ocio", "Servicios", "Deudas", "Reembolso"];
...
function activeMonthDate(day = "01") {
...
function hasFirebaseConfig(config = storedFirebaseConfig()) {
  return Boolean(
    config?.apiKey &&
...
function initializeFirebaseAuth() {
  ...
  if (!window.firebase?.initializeApp || !window.firebase?.auth) {
    authError = "No se pudo cargar Firebase. Revisa tu conexion a internet y recarga la app.";
    authReady = true;
    render();
    return;
  }
...
function ensureSeedCategories(current) {
...
function purgeDemoData(current) {
...
function migrateImportedAccountingMonth(current) {
...
function txMonth(tx) {
  return tx.accountingMonth || monthKey(tx.date);
}
...
function monthlyBaseline() {
  ...
  const totals = sampleMonths.map((key) => {
    ...
    const recs = state.receivables.filter((r) => monthKey(r.date) === key || monthKey(r.collectedDate || r.date) === key);
...
function monthFinance(key) {
  const txs = state.transactions.filter((tx) => txMonth(tx) === key);
  const receivables = state.receivables.filter((r) => monthKey(r.date) <= key && (r.status === "pending" || monthKey(r.collectedDate || r.date) === key));
...
function addTransaction(input) {
  const tx = {
    ...
    accountingMonth: input.accountingMonth || monthKey(input.date || activeMonthDate("01")),
...
function partialCollectReceivable(id) {
  ...
  if (!amount || amount <= 0) return alert("Ingresa un monto de abono valido.");
  if (amount >= current) {
...
function dedupeKey(row) {
  return [row.accountingMonth || monthKey(row.date), row.date, row.description.trim().toLowerCase(), Number(row.amount || 0).toFixed(2)].join("|");
}
...
function parseImportText(text) {
  ...
  lines.forEach((line) => {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(cleaned[0])) {
      ...
      dateRaw = activeMonthDate(cleaned[0]);
...
function normalizeImportDate(value, dayValue) {
  ...
  return dayValue ? activeMonthDate(dayValue) : activeMonthDate("01");
}
  ...
  return `
    <div class="topbar">
      ...
      <div class="actions">
        ...
        <label class="month-control"><span>Mes</span><input id="selectedMonth" type="month" value="${activeMonth()}"></label>
...
function renderImportStatusNotice() {
  ...
  const pending = state.importCandidates?.filter((item) => (item.accountingMonth || monthKey(item.date)) === activeMonth()) || [];
  if (pending.length) {
        ...
        <div class="table-wrap compact-table">
          <table>
            ...
            <tbody>
              ${rows.map((tx) => `
                <tr>
                  <td>${tx.date}${tx.accountingMonth && tx.accountingMonth !== monthKey(tx.date) ? `<br><span class="muted">Mes resumen ${tx.accountingMonth}</span>` : ""}</td>
...
function renderPaymentCalendar() {
  ...
  ].filter((item) => (item.accountingMonth || monthKey(item.date)) === activeMonth()).sort((a, b) => a.date.localeCompare(b.date)).slice(0, 8);
      ...
      <div class="card panel">
        <div class="panel-head">
          <div>
            ...
            <p class="muted">Marca cada movimiento del mes como permanente/obligatorio o pasajero. Las cuotas se proyectan solo por las cuotas restantes.</p>
          </div>
            ...
            ${rows.map((tx) => {
              ...
              return `
                <div class="projection-row">
                  ...
                  <select data-projection-type="${tx.id}">
  ...
  return `
    <section class="grid two-col">
      <div class="card panel">
        ...
        <p class="muted">Formato: fecha, descripcion, monto, tipo, categoria. El monto puede venir como $2.223, 2223, 2.223 o $2223; la app lo normaliza a CLP.</p>
      ...
      <div class="card panel">
        ...
        ${state.importCandidates.length ? `<div class="import-list">${state.importCandidates.map((c) => `
          <label class="candidate">
            ...
            <span><strong>${c.date} - ${escapeHtml(c.description)}</strong><br><span class="muted">${kindPill(c.kind)} ${distributionPill(c.distribution || inferDistribution(c))} ${money(c.amount)} ${c.duplicate ? "- duplicado detectado" : ""}</span></span>
            <span>${escapeHtml([c.category, c.bankName, c.account].filter(Boolean).join(" - "))}</span>
          </label>
...
function handleAction(action) {
  ...
  if (action === "save-projection") {
    ...
    saveState();
    alert("Proyeccion guardada. Puedes volver a ajustar las clasificaciones cuando quieras.");
  }
...
function readTemplateFile() {
  ...
  if (!file) return alert("Selecciona el archivo modelo completado.");
  ...
  reader.onload = () => {
    try {
      ...
      saveState();
      render();
      if (!candidates.length) alert("No se encontraron filas validas. Revisa encabezados y montos.");
    } catch (error) {
      alert(error.message);
    }
  };
